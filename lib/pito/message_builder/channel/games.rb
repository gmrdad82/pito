...
module Pito
  module MessageBuilder
    module Channel
      ...
      # and videos), the `games channel @handle` segment tool, and the `games`
      ...
      # with no linked games (tools.yml emit_if — mirrors videos) when games is
      # part of a combined view (`full`/`with …`). When games is the SOLE
      # segment requested (`games channel @h` / `only games`) and the guard
      # fails, Show#solo_empty_event emits a friendly one-liner
      # (pito.copy.channels.games_empty) instead of this builder, so the
      # caller never sees a bare empty Result (T16.14).
