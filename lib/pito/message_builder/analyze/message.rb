...
module Pito
  module MessageBuilder
    module Analyze
      ...
      # proving the fan-out + tool + with/without work end-to-end. Roles differ by
      ...
      module Message
        ...
        # Derives the segment-name → role mapping from the analyze tool's config.
        # Replaces the former SEGMENT_ROLES constant — now read from tools.yml.
        # Memoized per process (config is frozen at boot; Config.reload! in dev
        # clears Config, so we re-derive lazily on next access).
        def segment_roles
          @segment_roles ||= begin
            segs = Pito::Chat::Segments.for(tool: :analyze, entity: :channel)
