...
module Pito
  module FollowUp
    ...
    # declared in config/pito/tools.yml and read at runtime via
    # Pito::Dispatch::Matrix.  Handlers do NOT declare mode or actions in Ruby —
    # tools.yml is the sole source of truth.
    #
    ...
    class Handler
      ...
      # Config-driven availability gate — tools.yml (the Matrix, via the Registry)
      # is the SOLE source of truth for which reply tools a card accepts. No handler
      # keeps a literal allowlist: that drift is exactly what shadowed `game` on the
      # video card. `declared?` is true when `action` is a tool THIS reply_target
      ...
      # then dispatches the declared tool (special-case or delegate to ToolDelegator).
      def declared?(action)
