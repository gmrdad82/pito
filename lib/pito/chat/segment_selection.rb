...
module Pito
  module Chat
    # Parses the segment-selection clause shared by the +show+ / +analyze+ tools.
    ...
    #
    #   SegmentSelection.parse("show vid #123",                         tool: :show, entity: :vid)
    #   # => Selection(mode: :default, names: [...defaults...],     unknown: [], conflict: false)
    #
    #   SegmentSelection.parse("show vid #123 full",                    tool: :show, entity: :vid)
    #   # => Selection(mode: :full,    names: [...all...],          unknown: [], conflict: false)
    #
    #   SegmentSelection.parse("show vid #123 with at-a-glance",        tool: :show, entity: :vid)
    #   # => Selection(mode: :with,    names: [...defaults+req...], unknown: [], conflict: false)
    #
    #   SegmentSelection.parse("show game #123 only similar,channels",  tool: :show, entity: :game)
    #   # => Selection(mode: :only,    names: [...req...],          unknown: [], conflict: false)
    #
    #   SegmentSelection.parse("show game #123 without channels",              tool: :show, entity: :game)
    ...
    #   SegmentSelection.parse("analyze vid #1 with views,breakdowns",  tool: :analyze, entity: :vid,
    ...
    #   * Each token is validated against Pito::Chat::Segments.names(tool:, entity:);
    ...
    module SegmentSelection
      ...
      # @param tool             [Symbol]        the tool being dispatched (:show, :analyze, …).
      ...
      # @return [Selection]
      def parse(raw, tool:, entity:, extra_vocabulary: [])
        text = raw.to_s
        ...
        all_names       = Segments.names(tool: tool, entity: entity)
        default_names   = Segments.default_names(tool: tool, entity: entity)
        extra_vocab_set = extra_vocabulary.map { |t| t.to_s.downcase }.to_set
        alias_map       = Segments.alias_map(tool: tool, entity: entity)
      ...
      # tool+entity — the seam segment tools use to force a single
      ...
      # @param all_names      [Array<String>] all valid segment names for this tool+entity.
