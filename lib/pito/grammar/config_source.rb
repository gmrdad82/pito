...
module Pito
  module Grammar
    ...
    # (config/pito/tools.yml). This is the config-driven source of truth for the
    ...
    #
    # CHAT SPEC MAPPING
    #   Every tool entry in tools.yml that declares a `chat:` branch produces one
    #   Pito::Grammar::Spec with namespace :chat. Field mapping:
    #     name            → tool key (Symbol)
    #     aliases         → tool[:aliases] as Array<Symbol> (absent → [])
    #     slots           → chat[:slots] each converted by build_slot (see SLOT MAPPING)
    #     description_key → tool[:description] as String
    #     auth            → always :any (the tool-level YAML auth is a dispatch-routing
    #                       concept; Grammar Spec auth for chat is always :any, matching
    #                       the pre-migration hand-authored table behavior)
    #
    # SLASH SPEC MAPPING
    #   Every tool entry that declares a `slash:` branch produces one Spec with
    #   namespace :slash (replaces lib/pito/grammar/specs.rb and the
    #   per-handler `grammar do…end` blocks). Field mapping:
    #     name            → tool key (Symbol)
    #     aliases         → tool[:aliases] as Array<Symbol> (tool-level; the only
    #                       chat+slash tool, `help`, carries no aliases so this is
    ...
    #                       distinct from tool[:description], which feeds the chat spec)
    ...
    #   `resolver:` keys used in tools.yml's vocabularies section.
    module ConfigSource
      # Dynamic resolver lambdas, keyed by the resolver name declared in tools.yml.
      ...
      # Returns Array<Spec> for every tool in tools.yml that declares a `chat:` branch.
      # Order mirrors the tools.yml declaration order.
      def chat_specs
        Pito::Dispatch::Config.data.fetch(:tools, {}).filter_map do |name, tool|
          next unless tool.is_a?(Hash) && tool.key?(:chat)

          Spec.new(
            namespace:       :chat,
            name:            name.to_sym,
            aliases:         Array(tool[:aliases]).map { |a| a.to_s.to_sym },
            slots:           build_slots(tool[:chat][:slots]),
            description_key: tool[:description]&.to_s,
            auth:            :any
          )
        end
      end

      # Returns Array<Spec> for every tool in tools.yml that declares a `slash:`
      # branch. Order mirrors the tools.yml declaration order. See SLASH SPEC MAPPING.
      def slash_specs
        Pito::Dispatch::Config.data.fetch(:tools, {}).filter_map do |name, tool|
          next unless tool.is_a?(Hash) && tool.key?(:slash)
      ...
      # tools.yml. Order mirrors the YAML declaration order.
      def vocabularies
