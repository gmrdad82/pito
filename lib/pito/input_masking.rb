...
module Pito
  ...
  module InputMasking
    ...
    # `/login …` or its `/authenticate` alias (tool-bounded, any case). Both route
    ...
    # Mask everything after the tool (the entire secret payload), e.g.
    #   /login 123456 → /login ******
    def mask_secret(input)
      tool, rest = input.to_s.strip.split(/\s+/, 2)
