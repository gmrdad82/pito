...
`docs/plans/beta/04-project-workspace/specs/mcp-dev-kb-surface.md` **Reviewer
run:** 2026-05-04
...
1. **`first_heading` cap at 200 lines.** `list_docs` scans up to 200 lines per
   file looking for the first `# H1`; bails out and returns `""` past that. Spec
   didn't say.
...
4. **Slug sanitization extras.** Beyond the spec's literal `[a-z0-9-]` rule, the
   implementation collapses runs of hyphens (`a---b` → `a-b`) and strips
...
6. **`prefix` rejection.** `list_docs` rejects absolute prefixes (`/etc`) and
   `..`-traversal prefixes at the boundary, before any `Dir.glob`. Spec implied
   this for `read_doc` only.

### 3. Simplify candidates (cosmetic, not worth a dispatch)

- `app/mcp/tools/list_docs.rb:104` — the `&& !line.start_with?("## ")` clause in
  `first_h1` is dead because `"## "` doesn't satisfy `start_with?("# ")` (the
  third character differs: `'#'` vs the space the H1 needs). Drop the second
  clause.
- `app/mcp/tools/list_docs.rb:31-32` — the `limit = 50 if limit <= 0` line is
  ...
  Minor; behavior is acceptable today (positional defaults still apply when the
  kwarg is omitted).
- `app/mcp/tools/save_note.rb:67-72` — `unique_path`'s `loop` has no upper
  bound. Practical risk is zero (Mobile cannot trigger > 1 collision/sec on its
  own), but a defensive `break if suffix > 1000` would close the theoretical
  hole.
...
- `docs/notes/` will accumulate as Mobile captures notes during use. Desktop
  curates and prunes — no automation. The user may want a `bin/notes prune` or
  `bin/notes promote` helper later. Refinement-backlog item.
...
1. **Action:** `cd ~/Dev/pito && bin/dev`. Wait until Puma logs
   `Listening on http://0.0.0.0:3000` and Sidekiq is attached. **Expected:** No
   errors. The MCP HTTP Puma is serving on `:3001` (proxied by Cloudflare Tunnel
   to `mcp.pitomd.com`).
2. **Action:** Open `https://app.pitomd.com/dashboard` in a browser.
   **Expected:** Dashboard loads; charts render. (Confirms the Rails web Puma is
   healthy.)
3. **Action:**
   `curl -s -X POST https://mcp.pitomd.com -H 'Content-Type: application/json' -H "Authorization: Bearer $(rails runner 'puts McpAccessToken.first.plaintext_for_curl' 2>/dev/null || echo MISSING)" -d '{"jsonrpc":"2.0","id":1,"method":"tools/list"}' | head -200`.
   **Expected:** A JSON envelope listing all MCP tools — confirm `list_docs`,
   ...
   dashboard tools. (If the bearer-token incantation is wrong locally, it's fine
   to skip and rely on the Mobile-side test below; the point is to see the three
   tool names.)
...
5. **`list_docs` recent logs.** Prompt Mobile: _"What was I working on last
   ...
   `list_docs(name_pattern: "log.md", sort: "mtime_desc", limit: 5)`. The result
   includes (in this order, most-recent first) the channel-revamp log and the
   postgres-migration log; `last_modified_at` is ISO8601 UTC; `size_bytes` is an
   integer; `first_heading` previews the H1 of each log. Mobile likely follows
   up with `read_doc` on the top result — that is fine, step 8 covers it.
...
7. **`list_docs` includes CLAUDE.md.** Prompt: _"List the markdown files at the
   repo root."_ or _"What's in CLAUDE.md?"_ **Expected:** `list_docs` without a
   `prefix` (or with `prefix: ""`) returns a result set that includes
   `CLAUDE.md` as one of the rows. (Mobile may or may not chain into `read_doc`;
...
For each of these, the expected outcome is an MCP error envelope
(`isError: true`) with a clear message. **No file content** should be returned.
...
16. **Happy path with explicit slug.** Prompt: _"Save this thought as a note:
    ...
    where `<YYYY-MM-DD>` is today's UTC date and the time matches the moment of
    the call. **Verify:**
    - `ls docs/notes/` — exactly one new `.md` file plus `.gitkeep`.
    - `cat docs/notes/<that-file>.md` — content matches the prompt verbatim.
17. **No slug.** Prompt: _"Save this: 'Quick reminder — log this in the Phase 4
    plan tomorrow.'"_ (No slug hint.) **Expected:** File lands as
    `docs/notes/<timestamp>-note.md`. `cat` confirms the content.
18. **Repeat — distinct timestamps.** Prompt: _"Save it again with the same
    content."_ a few seconds later. **Expected:** A second file appears with a
    later timestamp; both notes from steps 16 and 18 are present in
...
20. **Slug that sanitizes to empty.** Prompt: _"Save 'punctuation only test'
    with title '!!!'."_ **Expected:** `slug: "!!!"` sanitizes to empty, falls
    back to `note`. File lands as `docs/notes/<timestamp>-note.md`.
...
22. **Sub-second collision (optional, advanced).** Two `save_note` calls in
    rapid succession with the same slug should produce two distinct files, one
    ending `-<slug>.md` and the other ending `-<slug>-2.md`. Hard to trigger
    from Mobile UX; acceptable to leave untested manually since the spec covers
    it (`save_note_spec.rb` exercises this with a frozen `Time`).
...
23. **Action:** Inspect `docs/notes/` and decide which captures are worth
    ...
    - **Promote a note into a log / ADR / spec.** Open the note in the editor,
      lift its content into the appropriate destination, then `rm` the source.
24. **Action:** `git status docs/notes/`. **Expected:** Either clean (if all
    ...
    intentionally). The architect commits `docs/notes/.gitkeep` only — the notes
    themselves stay out of git unless explicitly added.
...
- [ ] Steps 1–4 (pre-flight) green: `bin/dev` healthy, `mcp.pitomd.com`
      reachable, three new tools visible in `tools/list`, `docs/notes/` empty.
- [ ] Steps 5–9 (read happy paths) green: `list_docs` returns the right shape
      and includes / excludes `CLAUDE.md` per the rules; `read_doc` returns full
      file bodies with ISO8601 UTC timestamps.
...
- [ ] Concern 1 (lexical-vs-realpath spec deviation) acknowledged. User decides:
      amend the spec (recommended) or queue a follow-up to add a realpath check.
...
1. **Untracked files (the new code).** `git status` shows
   ...
   four new specs, and
   `docs/plans/beta/04-project-workspace/specs/mcp-dev-kb-surface.md` plus
   `additions.md` as untracked. Removing them is `rm <each>` — destructive, no
   git history involved.
2. **Tracked files modified.** `docs/mcp.md` and `CLAUDE.md` (and
   ...
   modified in the working tree but not yet committed. `git checkout --` on each
   restores the pre-Step-0 state. **Confirm with the user before running** —
   destructive.
3. **Captured notes.** Anything the user typed into `save_note` during manual
   testing lives in `docs/notes/`. Curate first, then `rm` the rest.
