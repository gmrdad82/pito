...
- [ ] T0.1 Create branch `reboot/beta` from `main`. model: [manual]
- [ ] T0.2 Tag current main as `v0.0.2-pre-reboot`. model: [manual]
- [ ] T0.3 Copy current Gemfile.lock to `docs/reboot/snapshots/Gemfile.lock.pre`. model: [manual]
- [ ] T0.4 Copy current schema to `docs/reboot/snapshots/schema.rb.pre`. model: [manual]
- [ ] T0.5 Dump current `app/` tree to `docs/reboot/snapshots/app-tree.txt`. model: [flash]
- [ ] T0.6 Dump current `config/locales/` tree to `docs/reboot/snapshots/locales-tree.txt`. model: [flash]
- [ ] T0.7 Stop dev services: kill `bin/dev`, `redis`, `meilisearch`, sidekiq workers. model: [manual]
- [ ] T0.8 Drop dev DB: `bin/rails db:drop`. model: [manual]
- [ ] T0.9 `docker compose down -v` to wipe Postgres/Redis/Meilisearch volumes. model: [manual]

## P1 — Rust CLI removal

- [ ] T1.1 Delete `extras/cli/` directory. model: [flash]
- [ ] T1.2 Delete `Cargo.toml` at repo root. model: [flash]
- [ ] T1.3 Delete `Cargo.lock` at repo root. model: [flash]
- [ ] T1.4 Delete `target/` build dir. model: [flash]
- [ ] T1.5 Delete `.deepseek/` (Rust agent dir). model: [flash]
- [ ] T1.6 Delete `docs/tui.md`. model: [flash]
- [ ] T1.7 Remove `Cargo.*` + `extras/cli/**` lines from `.gitignore` + `.dockerignore`. model: [flash]
- [ ] T1.8 Remove `extras/cli` path filter from `.github/workflows/*.yml`. model: [flash]
- [ ] T1.9 Remove "Rust TUI" references from `docs/architecture.md`. model: [haiku]
- [ ] T1.10 Remove "TUI" references from `docs/design.md`. model: [haiku]
- [ ] T1.11 Remove `rust` row from AGENTS.md skill table. model: [flash]
...
- [ ] T6.1 Remove `meilisearch` from Gemfile. model: [flash]
- [ ] T6.2 Remove `doorkeeper` from Gemfile. model: [flash]
- [ ] T6.3 Remove `mcp` from Gemfile. model: [flash]
- [ ] T6.4 Remove `chartkick` from Gemfile. model: [flash]
- [ ] T6.5 Remove `groupdate` from Gemfile. model: [flash]
- [ ] T6.6 Remove `aasm` from Gemfile. model: [flash]
- [ ] T6.7 Remove `friendly_id` from Gemfile. model: [flash]
- [ ] T6.8 Remove `commonmarker` from Gemfile. model: [flash]
- [ ] T6.9 Remove `dotenv-rails` from Gemfile. model: [flash]
- [ ] T6.10 Remove `pry-rails` from Gemfile (keep `debug` only). model: [flash]
- [ ] T6.11 Remove `google-apis-youtube_analytics_v2` from Gemfile (analytics out of v1 scope). model: [flash]
- [ ] T6.12 Keep: `jbuilder` (Rails 8 default), `rack-attack`, `image_processing`, `ruby-vips`, `neighbor`, `rotp`, `rqrcode`, `omniauth-google-oauth2`, `omniauth-rails_csrf_protection`, `google-apis-youtube_v3`. model: [manual]
- [ ] T6.13 Delete `config/initializers/doorkeeper.rb`, `doorkeeper_audit.rb`, `doorkeeper_layout.rb`, `doorkeeper_scope_clip.rb`. model: [flash]
- [ ] T6.14 Delete `config/initializers/friendly_id.rb`. model: [flash]
- [ ] T6.15 Remove `use_doorkeeper` + OAuth registration routes from `config/routes.rb`. model: [haiku]
- [ ] T6.16 Delete `app/controllers/oauth/` + `app/controllers/well_known_controller.rb`. model: [flash]
- [ ] T6.17 Delete files under `app/services/voyage/` only if they import Meilisearch (keep Voyage proper). model: [sonnet]
- [ ] T6.18 Search `app/` for `AASM`, `Chartkick`, `Groupdate`, `FriendlyId`, `Commonmarker`, `Meilisearch`, `Sidekiq`, `Doorkeeper`, `MCP` symbols; remove or stub. model: [sonnet]
- [ ] T6.19 `bundle install`; ensure `bin/rails runner "puts 1"` boots. model: [manual]
- [ ] T6.20 Commit: `[skipci] gem cull — doorkeeper, mcp, meilisearch, chartkick, groupdate, aasm, friendly_id, commonmarker, dotenv-rails, pry-rails`. model: [manual]
