# Phase 22 §6.2 — Channel::VideoImporter.
#
...
#
# Errors:
#   - Transient — `Channel::VideoImporter::TransientError` (re-raise
#     so Sidekiq retries; the job class wraps the raise).
#   - Permanent — `Channel::VideoImporter::FatalError` with
    ...
    def process_page(channel:, import_job:, items:)
      ...
      items.each do |item|
        ...
        if record.persisted?
          ...
          Rails.logger.warn(
            "[Channel::VideoImporter] failed to create video for #{yid}: #{record.errors.full_messages.join('; ')}"
          )
        end
      end
