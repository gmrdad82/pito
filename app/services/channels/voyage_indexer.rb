...
#
# Mirrors `Bundles::VoyageIndexer` + `Games::VoyageIndexer` for the
# Channel record so the unified search corpus + neighbor-lookup
# surfaces (e.g. `Games::ChannelRecommendation` via the `has_neighbors
...
#   `Bundles::VoyageIndexer`'s "bundle name + member-games summaries
...
#
# Gating: matches `Games::VoyageIndexer` —
...
module Channels
  class VoyageIndexer
    ...
    # budget under control. See `Bundles::VoyageIndexer
