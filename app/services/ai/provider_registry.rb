...
module Ai
  ...
  module ProviderRegistry
    ...
    # Raises unless +hash+'s keys are exactly +allowed+ (no unknown, none missing).
    def check_keys!(hash, allowed, path:)
      extra = hash.keys - allowed
      invalid!(path, "unknown key(s) #{extra.inspect} (allowed: #{allowed.inspect})") unless extra.empty?

      missing = allowed - hash.keys
      return if missing.empty?

      invalid!(path, "missing key(s) #{missing.inspect} (allowed: #{allowed.inspect})")
    end
