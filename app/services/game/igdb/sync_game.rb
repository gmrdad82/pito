  ...
  module Igdb
    class SyncGame
      ...
      def resolve_version_parent_id(version_parent_igdb_id, depth: 0)
        ...
        if depth >= 3
          Rails.logger.warn("[Game::Igdb::SyncGame] version_parent recursion depth exceeded (#{parent_igdb_id})")
          return existing&.id
        end

        parent_json = @client.fetch_game(parent_igdb_id).first
        if parent_json.nil?
          Rails.logger.warn("[Game::Igdb::SyncGame] version_parent id=#{parent_igdb_id} not found on IGDB")
          return existing&.id
        end
        ...
        if parent_json["version_parent"].present? && parent_json["version_parent"].to_i != parent_igdb_id
          Rails.logger.warn("[Game::Igdb::SyncGame] IGDB returned a chain at #{parent_igdb_id} → #{parent_json['version_parent']}; walking up")
      ...
      def sync_genres(game, genres_json)
        ...
        genre_records.each_with_index do |g, index|
          ...
          # (`Game::Igdb::SyncGame#re_assign_primary_genre`) runs the picker
