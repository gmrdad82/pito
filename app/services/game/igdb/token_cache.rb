...
# (called by `Game::Igdb::Client` on a 401, which retries the request once).
require "net/http"
require "json"
  ...
  module Igdb
    class TokenCache
      ...
      def handle(response)
        ...
        raise Game::Igdb::Client::AuthError, "Twitch token endpoint returned #{code}: #{response.body}" unless code == 200

        payload = parse_json(response.body)
        access_token = payload["access_token"]
        expires_in   = payload["expires_in"].to_i

        raise Game::Igdb::Client::AuthError, "Twitch returned no access_token" if access_token.blank?
      ...
      def parse_json(body)
        ...
        raise Game::Igdb::Client::AuthError, "Twitch returned malformed JSON: #{e.message}"
      end
    end
  end
end
