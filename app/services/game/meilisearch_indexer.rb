...
class Game
  class MeilisearchIndexer
    ...
    def call
      ...
      Rails.logger.warn("[Game::MeilisearchIndexer] upsert failed for game #{@game.id}: #{e.class}: #{e.message}")
    end
    ...
    def document
      ...
      # by `Game::Igdb::GameMapper#extract_alternative_names`. `respond_to?`
