...
#   3. For each `accept pito` field — call `Youtube::VideosReader` for
#      the fresh API snapshot (1 unit) and then
#      `Youtube::VideosClient#update_video(video, fresh:, fields: ...)`
  ...
  class VideoDiffApply
    ...
    def call
      ...
      ActiveRecord::Base.transaction do
        ...
        # request payload.
        if pito_fields.any?
          writable_pito_fields = pito_fields & Youtube::DiffComputer::WRITABLE_FIELDS
    ...
    def push_to_youtube!(writable_pito_fields)
      ...
      reader = @injected_reader || Youtube::VideosReader.new(connection)
      client = @injected_client || Youtube::VideosClient.new(connection)
