...
# `games_#{Rails.env}` index that `Game::MeilisearchIndexer` writes
...
# lists) is owned by `Game::MeilisearchIndexer` — both indexers write
...
class Bundle
  class MeilisearchIndexer
    ...
    def call
      ...
      Rails.logger.warn("[Bundle::MeilisearchIndexer] upsert failed for bundle #{@bundle.id}: #{e.class}: #{e.message}")
    end
    ...
    def push_document(url)
      ...
      # comment in `Game::MeilisearchIndexer#push_document` — both
