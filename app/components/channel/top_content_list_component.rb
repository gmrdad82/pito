...
#
# Mock data this slice — pulled from `Channel::MockData.top_content`
...
# already used on the `Channel::IdCardComponent` footer.
class Channel::TopContentListComponent < ViewComponent::Base
  # @param videos [Array<Hash>] mock video hashes — see
  #   `Channel::MockData.top_content` for the schema. Already filtered
  ...
  def thumbnail_box_style
    "width: 80px; height: 45px; flex-shrink: 0; border: 1px solid var(--color-border); border-radius: 0;"
  end
end
