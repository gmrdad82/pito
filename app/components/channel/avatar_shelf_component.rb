...
#
# Thin wrapper that lays out one `Channel::AvatarChipComponent` per
...
# (`Game::FilterRowComponent` filter_row_component.html.erb L23). The
...
# lives on `Channel::AvatarChipComponent` per the design.md rule
# ("avatar tile height = 2 × chip text line-height + the inter-row
# gap" — `docs/design.md` §"Channel avatars" L828-L831).
class Channel::AvatarShelfComponent < ViewComponent::Base
  # @param channels [Array<Hash>] channel hashes from
  #   `Channel::MockData.channels`. Each hash carries `:id`,
