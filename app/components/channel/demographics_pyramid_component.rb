...
class Channel::DemographicsPyramidComponent < ViewComponent::Base
  # @param channels [Array<Hash>] channel hashes from
  #   `Channel::MockData.channels`. Demographics may be inline on the
  #   hash (`:demographics`) or fetched lazily via
  #   `Channel::DemographicsMock.for(id)`.
  ...
  def profile
    @profile ||= Channel::DemographicsMock.aggregate(channels_with_demographics, weighted: weighted)
  end
  ...
  def max_pct
    @max_pct ||= [ Channel::DemographicsMock.max_cell(profile), 1 ].max
  end
  ...
  # `Channel::DemographicsMock.for`.
  def channels_with_demographics
    channels.map do |c|
      next c if c[:demographics]
      c.merge(demographics: Channel::DemographicsMock.for(c[:id]))
    end
  end
end
