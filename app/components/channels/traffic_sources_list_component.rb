...
#
# All inputs are the mock-data shape from `Channels::MockData`. Real
# data swap in Wave B will be a constant change at the call site.
#
# Inert — no Stimulus, no actions. CSS-only.
class Channels::TrafficSourcesListComponent < ViewComponent::Base
  MAX_SEARCH_TERMS = 10

  # @param channels [Array<Hash>] channel hashes from
  #   `Channels::MockData.channels`. Each must carry `:view_count`,
