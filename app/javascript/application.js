...
function scrambleText(el,newText,newClass){
  const oldText=el.textContent||'';
  if(oldText===newText){el.className=newClass;return;}
  let step=0,steps=5;
  const iv=setInterval(()=>{
    step++;
    el.textContent=newText.split('').map((c,i)=>(i>=oldText.length||c===oldText[i]||Math.random()>step/steps*0.7+0.3)?c:SCRAMBLE_CHARS[Math.floor(Math.random()*SCRAMBLE_CHARS.length)]).join('');
    if(step>=steps){el.textContent=newText;el.className=newClass;clearInterval(iv);}
  },30);
}
...
function draw(){
  ...
  if(c<20||r<5)return;
  const mw=c-(sidebarOpen?SW+1:0),sh=r-2;
  const out=[];
  let h='';
  ...
  for(let i=0;i<sh;i++){
    ...
    l=l.padEnd(mw);
    if(sidebarOpen&&c>SW+1){l+=S(' ',null,T.bd)+sbLine(i);}
    out.push(l);
  }
...
function sbLine(i){
  ...
  if(i===1)return S('  (use /channels)'.padEnd(sw),T.mu);
...
async function apiCmd(cmd){
  ...
  switch(a){
    ...
    case'status':try{const r=await fetch('/dashboard.json'),d=await r.json();log.push(S('dashboard:',T.fg)+B);log.push('  channels '+S(d.channel_count,T.gr));log.push('  videos   '+S(d.video_count,T.gr));log.push('  footage  '+S(d.footage_count,T.gr));}catch(e){log.push(S('  error: '+e.message,T.rd));}break;
    case'channels':try{const r=await fetch('/channels.json'),d=await r.json();channels=d;log.push(S('channels ('+d.length+'):',T.fg)+B);d.forEach(c=>log.push('  '+(c.star?S('★',T.ac):' ')+' '+S(c.channel_url,T.ac)));}catch(e){log.push(S('  error: '+e.message,T.rd));}break;
    case'videos':try{const r=await fetch('/videos.json'),d=await r.json();log.push(S('videos ('+d.length+'):',T.fg)+B);d.slice(0,30).forEach(v=>log.push('  '+v.youtube_video_id+' '+S('·',T.mu)+' '+S(v.views,T.gr)+' views'));}catch(e){log.push(S('  error: '+e.message,T.rd));}break;
    case'auth':if(!args[0]||args[0].length!==6){log.push(S('  usage: /auth <6-digit-code>',T.mu));break;}try{const r=await fetch('/login',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded','X-CSRF-Token':csrf()},body:'code='+args[0],redirect:'manual'});log.push((r.ok||r.status===302)?S('  authenticated',T.gr):S('  login failed',T.rd));}catch(e){log.push(S('  error: '+e.message,T.rd));}break;
    case'reindex':if(!args[0]||!['meilisearch','voyage'].includes(args[0])){log.push(S('  usage: /reindex meilisearch|voyage',T.mu));break;}try{const r=await fetch('/commands/execute',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-Token':csrf()},body:JSON.stringify({command:'reindex '+args[0]})}),d=await r.json();log.push(d.error?S('  '+d.error,T.rd):S('  '+d.output,T.gr));}catch(e){log.push(S('  error: '+e.message,T.rd));}break;
    case'games':try{const r=await fetch('/commands/execute',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-Token':csrf()},body:JSON.stringify({command:'games'})}),d=await r.json();if(d.error)log.push(S('  '+d.error,T.rd));else{log.push(S('games:',T.fg)+B);d.output.split('\n').forEach(l=>log.push('  '+l));}}catch(e){log.push(S('  error: '+e.message,T.rd));}break;
    case'config':try{const r=await fetch('/commands/execute',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-Token':csrf()},body:JSON.stringify({command:'config'})}),d=await r.json();log.push(d.error?S('  '+d.error,T.rd):S('  '+d.output,T.gr));}catch(e){log.push(S('  error: '+e.message,T.rd));}break;
