...
export default class extends Controller {
  ...
  // Footer-hint taps (touch parity for the palette-language footer chips —
  // ↑/↓, enter, ctrl+f, ctrl+x). pito--kbd-click's own HANDLERS map only
  // knows document-global chords (esc, ctrl+k, …); these are list-local, so
  // each hint's ShortcutComponent ALSO wires a click straight to one of
  // these public methods (see picker_component.html.erb). Every one acts on
  // the CURRENTLY SELECTED row — same semantics as the physical chord it
  // mirrors, reusing the exact same private logic the real keydown runs.
  // ↑/↓ is one hint for two keys; a single tap can't carry direction, so it
  // advances forward (mirrors ArrowDown) — repeat taps step through the list.
  hintMove() {
    this.#move(1);
  }
  ...
  hintEnter() {
    ...
    if (selected) this.#enter(selected);
  }
  ...
  filter() {
    ...
    this.rowTargets.forEach((row) => {
      ...
      const haystack =
        `${row.dataset.provider}/${row.dataset.value}`.toLowerCase();
  ...
  async #enter(row) {
    switch (row.dataset.rowType) {
      ...
      case "connect":
        return this.#revealKeyInput(row.dataset.provider);
      case "effort":
        return this.#cycleEffort(row);
    }
  }
  ...
  async #pick(row) {
    ...
    if (response && response.status === 422)
      return this.#flash("unknown model");
    ...
    this.rowTargets.forEach((r) => {
      ...
      if (marker)
        marker.textContent =
          r === row ||
          (r.dataset.provider === provider && r.dataset.value === model)
            ? "●"
            : "";
    });
  ...
  #revealKeyInput(provider) {
    const input = this.keyInputTargets.find(
      (i) => i.dataset.provider === provider,
    );
    if (!input) return;
  ...
  async #cycleEffort(row) {
    ...
    const next =
      EFFORT_CYCLE[(EFFORT_CYCLE.indexOf(current) + 1) % EFFORT_CYCLE.length];
    ...
    if (this.hasEffortValueTarget)
      this.effortValueTarget.textContent =
        next === "off" ? "model default" : next;
  ...
  async #toggleFavorite() {
    ...
    if (!response || !response.ok)
      return this.#flash("could not toggle favorite");
  ...
  #connectRow(provider) {
    return this.rowTargets.find(
      (r) => r.dataset.rowType === "connect" && r.dataset.provider === provider,
    );
  }

  #setChip(provider, present) {
    const chip = this.keyChipTargets.find(
      (c) => c.dataset.provider === provider,
    );
    if (!chip) return;
  ...
  #selectedRow() {
    return (
      this.rowTargets.find((r) =>
        r.classList.contains("pito-palette-selected"),
      ) || null
    );
  }
