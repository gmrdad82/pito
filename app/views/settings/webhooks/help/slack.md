...
webhook to receiving a pito test ping in your Slack channel.

No prior Slack-admin experience needed — if you can sign in to
Slack, you can follow this.

A Slack webhook is just a URL.

When something sends an HTTP POST to that URL, Slack writes the
...
— use the same account that has access to the Slack workspace you
want notifications to land in.

Click `Create New App` in the top right. A dialog asks how you'd
like to configure your new app — pick `From scratch`.
...
Webhooks". Flip it from `Off` to `On`.
...
Slack asks which channel pito should be allowed to post to. Pick
the channel — it can be a public channel, a private channel you
belong to, or a DM to yourself. Click `Allow`.

You bounce back to the "Incoming Webhooks" page. Near the bottom,
in the "Webhook URLs for Your Workspace" table, there's a new row
...
1. **Validates the URL shape.** It must start with
   `https://hooks.slack.com/services/` and have three slash-
   separated segments after `services/`.
2. **Sends a test message** to the channel. Only if Slack accepts
...
| **test ping failed: Slack returned 404 / 410** | the webhook was deleted in Slack               | re-run Step 3 and paste the new URL                     |
...
Clear the `webhook URL` field and click `[update]`. Pito removes
the saved configuration. Both toggles reset to off automatically.
