# Phase 25 — 01c. Web-surface `[block the intruder]` action screen for
# the new-location pending-approval flow.
#
# Two-action controller mirroring `Login::ApprovalsController`:
#
#   - GET  /login/blocks/:id — confirmation screen with attempt
#     detail. The `[block the intruder]` submit button POSTs to
#     `create`. Destructive (red) per CLAUDE.md's bracketed-link
#     convention.
#   - POST /login/blocks/:id — consumes the `confirm=yes` form, calls
#     `Auth::LoginAttemptBlocker` (which upserts a `BlockedLocation`,
#     revokes the pending session, resolves the linked notification,
#     and audit-logs).
#
# Yes/no boundary on `confirm` per CLAUDE.md. The action-screen
# framework forbids any JS confirm — the confirmation UX is the
# server-rendered screen itself.
class Login::BlocksController < ApplicationController
  before_action :load_attempt

  # GET /login/blocks/:id
  def show
    redirect_expired if expired_state?
  end

  # POST /login/blocks/:id
  def create
    return redirect_expired if expired_state?

    unless params[:confirm].to_s == "yes"
      redirect_to notifications_path, alert: "block cancelled."
      return
    end

    Auth::LoginAttemptBlocker.call(
      login_attempt: @attempt,
      acting_user: Current.user,
      source: :web,
      reason: params[:reason].to_s.presence,
      request: request
    )

    redirect_to notifications_path, notice: "blocked."
  rescue Auth::LoginAttemptBlocker::PendingExpired
    redirect_expired
  rescue Auth::LoginAttemptBlocker::AlreadyResolved
    redirect_to notifications_path,
                alert: "this login request was already resolved."
  end

  private

  def load_attempt
    @attempt = LoginAttempt.find_by(id: params[:id])

    if @attempt.nil? || @attempt.session_id.blank?
      redirect_to notifications_path, alert: "login request not found." and return
    end

    @session = Session.find_by(id: @attempt.session_id)
    if @session.nil?
      redirect_to notifications_path, alert: "login request not found." and return
    end
  end

  def expired_state?
    return true if @session.nil?
    !@session.state_pending_approval? || !@session.pending_within_window?
  end

  def redirect_expired
    redirect_to notifications_path,
                alert: "this login request has expired."
  end
end
