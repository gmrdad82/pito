...
# Beta migration — consolidates ALL prior migrations into one baseline.
# Generated from db/schema.rb on 2026-06-07 (schema version 2026_06_07_182924).
#
# Fresh installs: db:migrate runs this and creates the full schema.
# Existing installs (already have tables): the guard at the top of `up`
#   returns early so no data is lost.
# db:prepare / db:setup both use db:schema:load which is still preferred
#   for truly fresh environments.
class BetaMigration < ActiveRecord::Migration[8.1]
  def up
    # Skip if the schema is already present (existing install or db:schema:load was used).
  ...
  def down
    raise ActiveRecord::IrreversibleMigration, "Cannot reverse the beta baseline migration."
  end
end
