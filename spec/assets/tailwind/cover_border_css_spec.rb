...
#   * collection composite leading tile via `.collection-cover-composite`
#
...
RSpec.describe "application.css — 2026-05-17 cover thumbnail border", type: :asset do
  ...
  describe "1px border rule applied to every cover surface" do
    it "declares the combined `.game-cover-img, .tile-cover img, .collection-cover-composite` rule" do
      # Single selector list with all three hooks; allow whitespace
      # variation but lock the order so the rationale comment above the
      # rule stays accurate.
      expect(css).to match(
        /\.game-cover-img,\s*\.tile-cover\s+img,\s*\.collection-cover-composite\s*\{/
      )
    end

    it "the rule body is exactly `border: 1px solid var(--color-cover-border)`" do
      # Capture the body of the combined selector and assert the border
      # declaration sits inside it (and that no second `border:` line
      # has crept in).
      body = css[
        /\.game-cover-img,\s*\.tile-cover\s+img,\s*\.collection-cover-composite\s*\{([^}]*)\}/m,
        1
      ]
