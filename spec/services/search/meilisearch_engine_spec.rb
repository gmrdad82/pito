...
RSpec.describe Search::MeilisearchEngine, skip: ENV["CI"].present? && "requires Meilisearch" do
  ...
  before do
    ...
    begin
      client.index("channels_test").delete_all_documents
  ...
  describe "#index" do
    it "indexes a channel" do
      engine.index(channel)
      wait_for_tasks

      result = engine.search(Channel, "code kitchen")
      expect(result[:hits].size).to eq(1)
      expect(result[:hits].first[:id]).to eq(channel.id)
    end
  ...
  describe "#remove" do
    it "removes a channel from the index" do
      engine.index(channel)
      wait_for_tasks
    ...
    it "does not raise for missing records" do
      expect { engine.remove(channel) }.not_to raise_error
    end
  end

  describe "#reindex_all" do
    it "indexes all channels" do
      create(:channel, title: "summit sessions", description: "hiking")
      engine.reindex_all(Channel)
      wait_for_tasks

      result = engine.search(Channel, "")
      expect(result[:total]).to eq(Channel.count)
    end
    ...
    it "replaces existing documents" do
      engine.reindex_all(Channel)
      wait_for_tasks
      count_before = engine.search(Channel, "")[:total]

      engine.reindex_all(Channel)
      wait_for_tasks
      count_after = engine.search(Channel, "")[:total]
  ...
  describe "#search" do
    ...
    it "returns hits with highlights" do
      result = engine.search(Channel, "code")
    ...
    it "returns record objects" do
      result = engine.search(Channel, channel.title)
      hit = result[:hits].find { |h| h[:id] == channel.id }
      expect(hit).not_to be_nil
      expect(hit[:record]).to eq(channel)
    end

    it "supports pagination" do
      result = engine.search(Channel, "", page: 1, per_page: 1)
      expect(result[:hits].size).to be <= 1
    end

    it "supports filters on channels" do
      connected = create(:channel, :connected, title: "filtered channel")
      engine.reindex_all(Channel)
      wait_for_tasks

      result = engine.search(Channel, "", filters: { connected: true })
      ids = result[:hits].map { |h| h[:id] }
      expect(ids).to include(connected.id)
    end

    it "returns empty results for non-existent index" do
      # Search on a model whose index was never created
      result = engine.search(Channel, "nonexistent query xyz123")
      expect(result[:hits]).to be_empty
    end
  end
