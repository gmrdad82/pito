...
RSpec.describe Pito::InputMasking do
  ...
  describe ".config_credential_command?" do
    ...
    it "is false for non-credential /config forms" do
      non_credential = [
        "/config fx comet", "/config motion off", "/config me nickname=x",
  ...
  describe ".mask_config_credentials" do
    ...
    it "is a no-op for a non-credential /config (values shown in the clear)" do
      expect(described_class.mask_config_credentials("/config me nickname=Catalin"))
        .to eq("/config me nickname=Catalin")
    end
