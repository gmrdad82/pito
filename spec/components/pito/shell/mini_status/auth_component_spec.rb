...
RSpec.describe Pito::Shell::MiniStatus::AuthComponent do
  describe "default state" do
    it "defaults to unauthenticated (state: false)" do
      ...
      expect(node.to_html).to include("● unknown")
    end
  end
  ...
  describe "state: false (anonymous)" do
    it "renders the anonymous label in a red span" do
      ...
      expect(node.css("span.text-red").text).to include("● unknown")
    end
  ...
  describe "state: true (authenticated)" do
    it "renders the green '■ well known' label" do
      ...
      expect(green_span.text.strip).to eq("■ well known")
    end

    it "renders the 'well known' label when authenticated" do
      node = render_inline(described_class.new(state: true))
      expect(node.to_html).to include("■ well known")
    end

    it "does not render the anonymous label" do
      node = render_inline(described_class.new(state: true))
      expect(node.to_html).not_to include("● unknown")
    end
  end
end
