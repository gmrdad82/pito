...
RSpec.describe Pito::Shell::PullRefreshHintComponent, type: :component do
  ...
  # #2: the reveal is a taller ASCII block — ARROWS_BEFORE arrows, the shrug+copy
  # row, ARROWS_AFTER arrows, and the filled circle that arms the reload.
  it "renders ARROWS_BEFORE + ARROWS_AFTER arrow rows and a circle reload row" do
    inner = render_inline(described_class.new).css("template#pito-pull-refresh-hint").first.inner_html
    expected_arrows = described_class::ARROWS_BEFORE + described_class::ARROWS_AFTER
    expect(inner.scan("pito-pull-hint__arrow").size).to eq(expected_arrows)
