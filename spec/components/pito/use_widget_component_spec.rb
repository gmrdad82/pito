...
RSpec.describe Pito::UseWidgetComponent, type: :component do
  ...
  it "renders exactly one .pito-copy__btn (no fill/stage button — copy-only)" do
    expect(node.css(".pito-copy__btn").size).to eq(1)
    expect(node.at_css("[data-pito-use-widget-fill]")).to be_nil
    expect(node.at_css("[data-controller='pito--chat-prefill']")).to be_nil
  end

  it "has a separate empty Copied! feedback target as the LAST child" do
