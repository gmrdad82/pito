...
# of CompactComponent so the sparkline is a first-class, reusable component — no
# inline chart code in the host.)
RSpec.describe Pito::Analytics::Metric::SparklineComponent, type: :component do
  def braille?(text) = text.chars.any? { |c| c.ord.between?(0x2800, 0x28FF) }

  let(:cols) { Pito::Analytics::Metric::BaseComponent::COLS }
  ...
  it "subclasses BaseComponent (the 45-col braille canvas)" do
    expect(described_class.ancestors).to include(Pito::Analytics::Metric::BaseComponent)
  ...
  it "renders the dotted-paper bg with a floored baseline row" do
    ...
    expect(node.css(".pito-metric__bg-row").last.text).to include(Pito::Analytics::Metric::BaseComponent::BASELINE_DOT)
  end
  ...
  it "floors a minimal baseline for an all-zero series (always an x-axis line)" do
    ...
    expect(zero.css(".pito-metric__row").last.text).to include(Pito::Analytics::Metric::BaseComponent::BASELINE_DOT)
  end
