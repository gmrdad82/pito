...
# ============================================================================
# THE ADD-AN-MCP-TOOL PROOF  (G130 — the MCP analog of the add-a-tool proof)
# ============================================================================
# Proves that ADDING AN MCP TOOL is config-only. The owner's contract, extended
# to MCP: a new read-only MCP TOOL is a tools.yml `mcp:` block (or an
# `mcp_readers:` entry) and NOTHING ELSE — no Ruby dispatch-tool→MCP-tool table,
# no Registry edit, no endpoint edit. This file proves it.
#
# It invents a brand-new synthetic dispatch tool `almanac` carrying an `mcp:`
# block, plus a standalone `mcp_readers:` entry `pito_almanac_log`, injected as
# YAML through the test-only DispatchConfigInjection seam. With ZERO production
# edits it then proves:
#
...
#                  Pito::Dispatch::Router recognizes as the `almanac` dispatch tool
#                  and runs its handler. (T2.2 will prove the Executor BUILDS that
#                  string from a `pito_almanac(topic:)` tool call and calls this
#                  same Router path.)
#   4. THE POINT — Registry / Router / Schema are the unmodified shipped code.
#
# A real new MCP tool over an EXISTING read tool needs zero Ruby at all; this
# proof invents a whole dispatch tool, so it also adds one fixture handler —
# exactly as the add-a-tool (dispatch) proof does.
...
RSpec.describe "the add-an-MCP-tool proof (G130)", type: :dispatch do
  ...
  describe "execution (the tool's grammar template dispatches)" do
    it "the input template, once filled, parses as the almanac verb" do
      expect(parse("almanac weather").tool).to eq(:almanac)
    end
    ...
    it "an input_suffix clause also dispatches (almanac weather with tides)" do
      expect(parse("almanac weather with tides").tool).to eq(:almanac)
    end
  ...
  describe "THE POINT — zero production code was touched" do
    ...
    it "the ONLY hand-written code the new tool needed is the fixture handler (in this spec)" do
      ...
      expect(source).to end_with("spec/dispatch/add_an_mcp_tool_proof_spec.rb")
    end
  end
end
