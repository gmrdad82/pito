...
# NEVER reached for /resume. Extra positional args produce :unknown_tool,
# NOT :too_many_args.
RSpec.describe "Dispatch matrix — resume (recognition, DB mocked)", type: :dispatch do
  ...
  describe "grammar recognition" do
    [
      ...
      it "#{input.inspect} → stack :slash, verb :resume, known: true" do
        ...
        expect(intent[:tool]).to eq(:resume)
  ...
  describe "dispatcher — handler-less (unknown_tool)" do
    {
      ...
      context "#{label} (#{raw.inspect})" do
        ...
        it "message_args includes tool: :resume" do
          expect(dispatch(raw).message_args[:tool]).to eq(:resume)
        end
      end
    end
  end
  ...
  describe "extra args → unknown_tool (arity guard not applicable)" do
    it "/resume foo → Result::Error with unknown_tool (not too_many_args)" do
