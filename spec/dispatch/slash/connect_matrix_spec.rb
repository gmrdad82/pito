...
# ── Dispatch matrix: `/connect` (recognition, DB mocked) ──────────────────────
#
# RULE: every input form the grammar recognises — no exception. Zero factories,
# zero DB writes. Real OAuth is NEVER triggered — these specs operate at the
# grammar / Dispatcher layer only.
#
# `/connect` is a handler-less slash command: it has a grammar spec
# (auth: :authenticated_only, zero slots) but NO handler class in
# Pito::Slash::Registry. In production the controller intercepts it
# synchronously via `connect_command?` → `handle_connect` (which initiates the
# OAuth flow). The Dispatcher is never called.
#
...
# NEVER reached for /connect. Extra positional args produce :unknown_verb,
# NOT :too_many_args.
RSpec.describe "Dispatch matrix — connect (recognition, DB mocked)", type: :dispatch do
  ...
  describe "grammar recognition" do
    [
      ...
      it "#{input.inspect} → stack :slash, verb :connect, known: true" do
        intent = parsed_intent(input)
        expect(intent[:stack]).to eq(:slash)
        expect(intent[:verb]).to eq(:connect)
        expect(intent[:known]).to be(true)
      end
    end

    it "auth tier is :authenticated_only (bare verb)" do
      expect(parsed_intent("/connect")[:auth]).to eq(:authenticated_only)
    end
  ...
  #
  # No handler class is registered for :connect. When the Dispatcher is called
  # directly it returns Result::Error(:unknown_verb) for every non-help input.
  # No OAuth URL is produced, no session is touched.
  # ═══════════════════════════════════════════════════════════════════════════
  describe "dispatcher — handler-less (unknown_verb)" do
    {
      "bare verb"           => "/connect",
      "trailing spaces"     => "/connect   ",
      "extra arg"           => "/connect foo",
      "multiple extra args" => "/connect foo bar"
    }.each do |label, raw|
      context "#{label} (#{raw.inspect})" do
        ...
        it "message_args includes verb: :connect" do
          expect(dispatch(raw).message_args[:verb]).to eq(:connect)
        end
      end
    end
  end
  ...
  # successful handler class lookup. Since /connect has no handler class, extra
  # positional args fall through to the unknown_verb branch instead.
  # This also confirms that no OAuth is initiated by extra args.
  # ═══════════════════════════════════════════════════════════════════════════
  describe "extra args → unknown_verb (arity guard not applicable)" do
    it "/connect foo → Result::Error with unknown_verb (not too_many_args)" do
      result = dispatch("/connect foo")
      expect(result).to be_a(Pito::Slash::Result::Error)
      expect(result.message_key).to eq("pito.slash.errors.unknown_verb")
    end

    it "/connect foo → message_key is NOT too_many_args" do
      expect(dispatch("/connect foo").message_key).not_to eq("pito.slash.errors.too_many_args")
    end
  end
  ...
  # BEFORE the controller's `connect_command?` guard (the controller's
  # `help_flag?` guard routes --help forms to the async Dispatcher instead of
  # the OAuth fast-path). No OAuth is initiated for any --help form.
  #
  ...
  describe "--help / -h intercept (Dispatcher-level)" do
    ...
    {
      "bare --help" => "/connect --help",
      "short -h"    => "/connect -h"
    }.each do |label, raw|
      context "#{label} (#{raw.inspect})" do
        it "calls HelpBuilder (handler lookup and OAuth bypassed)" do
    ...
    it "no leading space: /connect--help → HelpBuilder NOT called (regex requires \\s)" do
      # The lexer may tokenize /connect--help as a single unknown verb → Result::Error;
      # either way, HelpBuilder is never reached and OAuth is never initiated.
      dispatch("/connect--help")
