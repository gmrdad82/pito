...
# ── Segment tools (plan-0.9.5 D20/D21) ──────────────────────────────────────
#
# Seven top-level chat tools, each promoting ONE :enhanced segment of
# show/analyze into its own tool via the single generic
# Pito::Chat::Handlers::SegmentTool + a `segment_of` config binding — additive;
...
#   * recognition — every tool + alias parses to its canonical tool, and the
#     noun-filler words that also became tools (videos/vids/channels) still parse
#     to their ORIGINAL tool at position ≥2 (`list videos`, `analyze channels`);
#   * byte-identity — each tool×valid-entity emits EXACTLY the parent's
...
#   * palette — the tool + its slot surface from config (no hand-maintained list).
RSpec.describe "segment tools (D20/D21)", type: :dispatch do
  ...
  # ── recognition ────────────────────────────────────────────────────────────
  describe "recognition — canonical tools + aliases at position 1" do
    {
      ...
      it "#{token.inspect} → tool #{tool.inspect}, a :new_turn" do
        msg = parse("#{token} game 1")
        expect(msg.kind).to eq(:new_turn)
        expect(msg.tool).to eq(tool)
      end
    end
  end

  describe "recognition — negatives: the filler/vocabulary words stay their original tool at position ≥2" do
    {
      "list videos"      => :list,
      "list vids"        => :list,
      "ls videos"        => :list,
      ...
      it "#{input.inspect} still parses to #{tool.inspect} (position-1 tool match beats position-n filler)" do
        expect(parse(input).tool).to eq(tool)
      end
    end
  end

  # ── byte-identity: tool×valid-entity == the parent's only-<segment> form ─────
  describe "byte-identity with the parent's only-<segment> emission" do
    ...
    # [segment-tool input, equivalent parent `only <segment>` input]
    def cases
      {
    ...
    it "each tool emits exactly the parent's only-<segment> events" do
      aggregate_failures do
        cases.each do |label, (seg_input, parent_input)|
          ...
          expect(seg.events).to eq(parent.events), "#{label}: segment-tool events differ from `#{parent_input}`"
  ...
  describe "off-entity rejection matches the parent's only-<segment> rejection" do
    ...
    # the video branch (a pre-existing show quirk). The segment tool resolves the
    ...
    it "`games game #id` rejects — games is a channel-only segment (G120)" do
      ...
      expect(result.message_key).to eq(Pito::Copy.render(
        ...
        tokens: "games",
        names:  Pito::Chat::Segments.names(tool: :show, entity: :game).join(", ")
      ))
    end
  end
  ...
  describe "segment_of schema validation" do
    ...
    # `similar` game segment, plus a segment tool pointing at it.
    def document(segment_of)
      {
        ...
        tools: {
          ...
          mytool: {
            chat: { dispatch: "Chat::Handlers::SegmentTool", segment_of: segment_of }
          }
        }
      }
    end
    ...
    it "accepts a binding to an existing parent segment" do
      expect(errors_for({ tool: "show", segment: "similar" })).to eq([])
    end

    it "rejects a parent tool that declares no segments block" do
      expect(errors_for({ tool: "analyze", segment: "similar" }).join)
        .to match(/segment_of\.tool .*not a tool declaring a segments block/)
    end

    it "rejects a segment name absent from the parent" do
      expect(errors_for({ tool: "show", segment: "nope" }).join)
        .to match(/segment_of\.segment "nope" is not a segment of show/)
    end

    it "rejects unknown keys inside segment_of" do
      expect(errors_for({ tool: "show", segment: "similar", extra: 1 }).join)
        .to match(/segment_of\.extra: unknown key/)
    end

    it "rejects a missing required key" do
      expect(errors_for({ tool: "show" }).join).to match(/segment_of\.segment: missing required key/)
    end
  ...
  describe "palette derives from config" do
    ...
    it "surfaces all seven segment tools" do
  ...
  #
  # ONE keyed tool where the NOUN names WHAT YOU GET (the segment) and the id is
  ...
  # The one-word game / videos segment tools AND link/unlink are unaffected.
  describe "the `linked` keyed tool" do
    describe "recognition — `linked` at position 1, distinct from link / linked-*" do
      { "linked game 1" => :linked, "linked vids 1" => :linked, "linked videos 1" => :linked }.each do |input, tool|
        it "#{input.inspect} → #{tool.inspect}, a :new_turn" do
          msg = parse(input)
          expect(msg.kind).to eq(:new_turn)
          expect(msg.tool).to eq(tool)
        end
      ...
      # Negatives: `link`/`unlink` and the one-word segment tools keep their tool —
      # the token distinction `link` ≠ `linked` and `linked` ≠ `linked-game` holds.
      {
        ...
        it "#{input.inspect} still parses to #{tool.inspect} (link ≠ linked; hyphenated tools distinct)" do
          expect(parse(input).tool).to eq(tool)
        end
      end
    end
    ...
    describe "keyed segment_of schema validation" do
      ...
      # plus a keyed segment tool pointing at them.
      ...
      it "accepts a per-noun keyed binding (entity-forced, with aliases)" do
        expect(keyed_errors(
          game: { tool: "show", segment: "linked-game",   entity: "vid" },
          vids: { tool: "show", segment: "linked-videos", entity: "game", aliases: [ "videos" ] }
        )).to eq([])
      end

      it "rejects an unknown entity in a branch" do
        expect(keyed_errors(game: { tool: "show", segment: "linked-game", entity: "nope" }).join)
