...
RSpec.describe Game, type: :model do
  ...
  describe "default title" do
    it 'defaults to "Untitled game"' do
      ...
      game = Game.create!(tenant: tenant)
  ...
  describe "platforms validation" do
    ...
    it "accepts the canonical example shape" do
      game = build(:game, tenant: tenant,
                          platforms: [ { "platform" => "PS5", "owned" => true, "recorded_on" => true } ])
      expect(game).to be_valid
    end

    it "accepts an empty array" do
      game = build(:game, tenant: tenant, platforms: [])
      expect(game).to be_valid
    end
    ...
    Game::ALLOWED_PLATFORMS.each do |platform|
      it "accepts #{platform.inspect}" do
        game = build(:game, tenant: tenant,
                            platforms: [ { "platform" => platform, "owned" => true, "recorded_on" => false } ])
        expect(game).to be_valid
      end
    end
