...
RSpec.describe ChatImportVideosJob, type: :job do
  ...
  describe "#perform" do
    ...
    context "with a conversation_id" do
      ...
      it "includes import_videos copy in the summary text" do
        broadcaster = instance_double(Pito::Stream::Broadcaster, emit: nil, complete_turn: nil)
        allow(Pito::Stream::Broadcaster).to receive(:new).and_return(broadcaster)

        described_class.new.perform([ channel.id ], "@pito", conversation_id: conversation.id)

        emitted_payload = broadcaster.instance_variable_get(:@conversation) # no-op; the check is in the broadcaster call
        expect(broadcaster).to have_received(:emit) do |args|
          expect(args[:payload]["text"]).to be_present
        end
      end
    end
  end
