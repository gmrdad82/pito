...
RSpec.describe SyncChannelJob, type: :job do
  ...
  describe "#perform" do
    ...
    it "does not require a conversation_id to run the sync" do
      expect { described_class.new.perform([ channel.id ], "@pito") }.not_to raise_error
      expect(ChannelSync).to have_received(:perform_now)
    end
