...
RSpec.describe Ai::Wire::AnthropicMessages, type: :service do
  ...
  describe "#chat" do
    ...
    context "streaming (block given)" do
      ...
      # One full Anthropic SSE stream: a text block, then a pito_respond
      # tool_use block whose input arrives as three input_json_delta
      # fragments, then the closing envelope events.
      let(:sse_events) do
        [
          [ "message_start",
            { type: "message_start",
          ...
          [ "content_block_start",
            { type: "content_block_start", index: 1,
              content_block: { type: "tool_use", id: "toolu_1", name: "pito_respond", input: {} } } ],
          [ "content_block_delta",
            { type: "content_block_delta", index: 1,
              delta: { type: "input_json_delta", partial_json: '{"blocks":' } } ],
          [ "content_block_delta",
            { type: "content_block_delta", index: 1,
              delta: { type: "input_json_delta", partial_json: '[{"type":"paragraph"}' } } ],
          [ "content_block_delta",
            { type: "content_block_delta", index: 1,
      ...
      let(:sse_body) do
        sse_events.map { |name, data| "event: #{name}\ndata: #{data.to_json}\n\n" }.join
      end

      def stub_stream(body: sse_body)
        stub_request(:post, endpoint).to_return(status: 200, body: body, headers: sse_headers)
      end

      it "adds stream: true to the request body" do
        stub = stub_request(:post, endpoint).with { |request|
          JSON.parse(request.body)["stream"] == true
        }.to_return(status: 200, body: sse_body, headers: sse_headers)

        build_adapter.chat(messages: [ { role: "user", content: "hey" } ], model: "claude-x") { |_name, _fragment| }

        expect(stub).to have_been_requested
      end

      it "yields each input_json_delta fragment in order, tagged with the tool name" do
        stub_stream

        fragments = []
        build_adapter.chat(messages: [ { role: "user", content: "hey" } ], model: "claude-x") do |name, fragment|
          fragments << [ name, fragment ]
        end

        expect(fragments).to eq([
          [ "pito_respond", '{"blocks":' ],
          [ "pito_respond", '[{"type":"paragraph"}' ],
          [ "pito_respond", "]}" ]
        ])
      end

      it "assembles the same Response shape as the non-stream path" do
        stub_stream

        response = build_adapter.chat(messages: [ { role: "user", content: "hey" } ], model: "claude-x") { |_name, _fragment| }

        expect(response.text).to eq("Working on it.")
        expect(response.tool_calls.size).to eq(1)
        tool_call = response.tool_calls.first
        expect(tool_call.id).to eq("toolu_1")
        expect(tool_call.name).to eq("pito_respond")
        expect(tool_call.arguments).to eq({ "blocks" => [ { "type" => "paragraph" } ] })
        expect(response.usage.input_tokens).to eq(25)
        expect(response.usage.output_tokens).to eq(40)
        expect(response.usage.total).to eq(65)
        expect(response.stop_reason).to eq(:stop)
      end

      it "reports streamed usage to Pito::Stack" do
        stub_stream

        expect(Pito::Stack).to receive(:track).with("ai", hash_including(units: 65))

        build_adapter.chat(messages: [ { role: "user", content: "hey" } ], model: "claude-x") { |_name, _fragment| }
      end

      it "leaves the no-block request body without a stream key" do
        stub = stub_request(:post, endpoint).with { |request|
          !JSON.parse(request.body).key?("stream")
        }.to_return(status: 200, body: minimal_success_body, headers: json_headers)
      ...
      it "raises Ai::Wire::Error on a non-2xx before consuming any event" do
        stub_request(:post, endpoint).to_return(status: 429, body: "rate limited")

        expect {
          build_adapter.chat(messages: [], model: "claude-x") { |_name, _fragment| raise "block must not run" }
