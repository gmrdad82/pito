...
RSpec.describe Pito::Share::UniversalActions do
  ...
  describe "tool constants" do
    ...
    it ".tools derives from Matrix.universal_tokens (share, revoke, unshare — no help)" do
      expect(described_class.tools).to match_array(%w[share revoke unshare])
      expect(described_class.tools).not_to include("help")
    end
  end
  ...
  describe ".tools_for — kind gating (owner ruling 2026-07-03)" do
    def event_of_kind(k)
      Event.create_with_position!(
    ...
    it "offers `share` for a non-shared :system message (no help — G92)" do
      expect(described_class.tools_for(event)).to match_array(%w[share])
    end
    ...
    it "offers `share` for a non-shared :system_follow_up message (owner D14: follow-ups shareable)" do
      ...
      expect(described_class.tools_for(ev)).to include("share")
      expect(described_class.tools_for(ev)).not_to include("help")
    end
    ...
    it "offers `share` for a non-shared :enhanced message" do
      ...
      expect(described_class.tools_for(ev)).to match_array(%w[share])
    end

    it "adds revoke/unshare once the event has a Share" do
      Share.find_or_create_by!(event:) { |s| s.conversation = conversation }
      expect(described_class.tools_for(event)).to match_array(%w[share revoke unshare])
    end
    ...
    it "offers nothing ([]) for a :confirmation message (G92: no help + no share)" do
      ...
      expect(described_class.tools_for(ev)).to eq([])
    end
    ...
    it "offers `share` for a nil event (the generic event-less help page)" do
      expect(described_class.tools_for(nil)).to match_array(%w[share])
    end
  end
  ...
  describe "resolution gate (unresolved messages are not shareable)" do
    ...
    it ".tools_for offers nothing ([]) while the message is unresolved (share tools withheld, no help)" do
      thinking_for(event, resolved: false)
      expect(described_class.tools_for(event)).to eq([])
    end

    it ".tools_for offers `share` once the message resolves (no help — G92)" do
      thinking_for(event, resolved: true)
      expect(described_class.tools_for(event)).to match_array(%w[share])
    end
    ...
    # G92 (2026-07-05): `help` is no longer a UniversalActions tool; `#call` with
    # rest "help" hits the `else` branch (unknown tool) and returns Result::Error
    # regardless of resolution state.
    it "#call returns Result::Error for `help` (now an unknown tool, not a universal action)" do
      thinking_for(event, resolved: false)
  ...
  describe "#call — unshare tool (alias for revoke)" do
    context "when a Share exists for the event" do
      before { Share.create!(event:, conversation:) }
  ...
  # G92 (2026-07-05): `help` removed as a UniversalActions tool; `#call` with rest
  # "help" now hits the `else` branch (unknown tool) exactly like any other
  ...
  describe "#call — help is now an unknown tool (G92)" do
    it "returns Result::Error for rest: 'help' (same as any unrecognised token)" do
      result = handler.call(source_event: event, rest: "help", conversation:)
      expect(result).to be_a(Pito::FollowUp::Result::Error)
    end

    it "returns the same error copy key as other unknown tools" do
