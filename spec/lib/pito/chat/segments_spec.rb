...
RSpec.describe Pito::Chat::Segments do
  let(:show_channel) { described_class.for(tool: :show, entity: :channel) }
  let(:show_vid)     { described_class.for(tool: :show, entity: :vid) }
  let(:show_game)    { described_class.for(tool: :show, entity: :game) }

  # ── .names ───────────────────────────────────────────────────────────────────

  describe ".names" do
    it "returns ordered names for show channel: detail, games, videos, at-a-glance" do
      expect(described_class.names(tool: :show, entity: :channel))
        .to eq(%w[detail games videos at-a-glance])
    end

    it "returns ordered names for show vid: detail, game, at-a-glance" do
      expect(described_class.names(tool: :show, entity: :vid))
        .to eq(%w[detail game at-a-glance])
    end

    it "returns ordered names for show game: detail, similar, videos, channels, at-a-glance" do
      expect(described_class.names(tool: :show, entity: :game))
  ...
  describe ".default_names" do
    it "returns [detail] for show channel" do
      expect(described_class.default_names(tool: :show, entity: :channel)).to eq(%w[detail])
    end

    it "returns [detail] for show vid" do
      expect(described_class.default_names(tool: :show, entity: :vid)).to eq(%w[detail])
    end

    it "returns [detail] for show game" do
      expect(described_class.default_names(tool: :show, entity: :game)).to eq(%w[detail])
    end
  end
  ...
  describe "frozen table" do
    it ".for returns a frozen array" do
      expect(described_class.for(tool: :show, entity: :game)).to be_frozen
    end

    it "mutating the returned array raises FrozenError" do
      segments = described_class.for(tool: :show, entity: :game)
      expect { segments << :extra }.to raise_error(FrozenError)
    end
  end

  # ── ArgumentError on unknown tool / entity ───────────────────────────────────

  describe ".for error handling" do
    it "raises ArgumentError for an unknown tool" do
      expect { described_class.for(tool: :bogus, entity: :game) }
        .to raise_error(ArgumentError, /unknown tool/)
    end

    it "raises ArgumentError for an unknown entity on a known tool" do
      expect { described_class.for(tool: :show, entity: :playlist) }
        .to raise_error(ArgumentError, /unknown entity/)
    end

    it "raises ArgumentError for an unknown entity on :analyze" do
      expect { described_class.for(tool: :analyze, entity: :playlist) }
        .to raise_error(ArgumentError, /unknown entity/)
    end
  end
  ...
  describe "analyze tool" do
    let(:analyze_channel) { described_class.for(tool: :analyze, entity: :channel) }
    let(:analyze_vid)     { described_class.for(tool: :analyze, entity: :vid) }
    let(:analyze_game)    { described_class.for(tool: :analyze, entity: :game) }

    describe ".names" do
      it "returns [numbers, breakdowns] for analyze channel" do
        expect(described_class.names(tool: :analyze, entity: :channel)).to eq(%w[numbers breakdowns])
      end

      it "returns [numbers, breakdowns] for analyze vid" do
        expect(described_class.names(tool: :analyze, entity: :vid)).to eq(%w[numbers breakdowns])
      end

      it "returns [numbers, breakdowns] for analyze game" do
        expect(described_class.names(tool: :analyze, entity: :game)).to eq(%w[numbers breakdowns])
      end
    end
    ...
    describe ".default_names" do
      it "returns [numbers] (the system segment) for all three entities" do
        %i[channel vid game].each do |entity|
          expect(described_class.default_names(tool: :analyze, entity:)).to eq(%w[numbers])
        end
      end
    end
  ...
  describe "segment aliases" do
    ...
    describe ".alias_map" do
      it "maps 'similars' → 'similar' for show/game" do
        map = described_class.alias_map(tool: :show, entity: :game)
      ...
      it "has canonical names plus the videos alias (vids) as keys for show/channel" do
        map = described_class.alias_map(tool: :show, entity: :channel)
  ...
  describe "analyze segment ↔ metric token disjointness" do
    it "analyze segment names do not collide with any MetricSelection token (canonical keys + aliases)" do
      segment_names = described_class.names(tool: :analyze, entity: :vid).to_set
