...
RSpec.describe Pito::Chat::SegmentSelection do
  # Default to tool: :show, entity: :game (5-segment table, richest coverage).
  def parse(raw, tool: :show, entity: :game)
    described_class.parse(raw, tool: tool, entity: entity)
  end
  ...
  # ── extra_vocabulary pass-through (analyze tool) ─────────────────────────────
  ...
  describe "extra_vocabulary pass-through (tool: :analyze)" do
    let(:metric_vocab) do
    ...
    def analyze_parse(raw)
      described_class.parse(raw, tool: :analyze, entity: :vid, extra_vocabulary: metric_vocab)
    end
    ...
    it "without extra_vocabulary, metric tokens land in unknown as before" do
      result = described_class.parse("analyze vid #1 with views", tool: :analyze, entity: :vid)
  ...
  describe "entity: :channel" do
    it "full returns all channel segment names in table order" do
      result = described_class.parse("show channel full", tool: :show, entity: :channel)
      expect(result.names).to eq(%w[detail games videos at-a-glance])
    end

    it "bare command returns default names (detail only)" do
      result = described_class.parse("show channel @handle", tool: :show, entity: :channel)
      expect(result.names).to eq(%w[detail])
    end

    it "'with vids' resolves to the canonical 'videos' segment (vids alias)" do
      result = described_class.parse("show channel @h with vids", tool: :show, entity: :channel)
      expect(result.mode).to eq(:with)
  ...
  describe "entity: :vid" do
    it "full returns all vid segment names in table order" do
      result = described_class.parse("show vid full", tool: :show, entity: :vid)
      expect(result.names).to eq(%w[detail game at-a-glance])
    end

    it "bare command returns default names (detail only)" do
      result = described_class.parse("show vid #1", tool: :show, entity: :vid)
      expect(result.names).to eq(%w[detail])
    end
  end
  ...
  # ── extra_vocabulary pass-through for :without (analyze tool) ─────────────────

  describe "extra_vocabulary pass-through with 'without' introducer (tool: :analyze)" do
    let(:metric_vocab) do
      Pito::Analytics::MetricSelection::ALIASES.keys +
        Pito::Analytics::MetricOrder::METRICS.keys.map(&:to_s)
    end
