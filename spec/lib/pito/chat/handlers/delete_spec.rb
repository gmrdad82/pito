...
RSpec.describe Pito::Chat::Handlers::Delete do
  ...
  def handler_for(*words)
    described_class.new(
      message: Pito::Chat::Message.new(tool: :delete, body_tokens: tokens(*words), kind: :new_turn, raw: "delete #{words.join(' ')}"),
  ...
  context "delete video" do
    ...
    it "does NOT delete the video yet (confirmation only)" do
      expect { handler_for("video", "##{video.id}").call }.not_to change(Video, :count)
    end
