...
RSpec.describe Pito::Suggestions::Engine, type: :service do
  ...
  describe "slash mode — tool stage" do
    ...
    context "menu_item shape" do
      ...
      it "masked is false for tool-stage items" do
        result = call(input: "/", cursor: 1, authenticated: true)
        result[:menu_items].each do |item|
  ...
  describe "slash mode — arg stage (/config provider slot)" do
    # Empty partial → namespace drill-down (ai/sources/profile), not a flat
    # provider spill; the providers live under each namespace row's children.
    ...
    it "matches a namespace by prefix too — '/config sour' offers the sources drill-down" do
      items = described_class.call(input: "/config sour", cursor: "/config sour".length, authenticated: true)[:menu_items]
    ...
    it "insert for a provider ends with a space (children of the namespace rows)" do
      ...
      result[:menu_items].each do |item|
        item[:children].each do |child|
          expect(child[:insert]).to end_with(" ")
        end
      end
    end
    ...
    # The provider list is browsable: tagged stage: :tool so the client renders
    # the whole set as a selectable palette (not just the top hit as a ghost).
    it "tags the provider slot stage: :tool (palette, not arg ghost)" do
      result = call(input: "/config ", cursor: 8, authenticated: true)
      expect(result[:stage]).to eq(:tool)
    end

    it "keeps stage: :tool while filtering providers by prefix ('/config g')" do
      result = call(input: "/config g", cursor: 9, authenticated: true)
      expect(result[:stage]).to eq(:tool)
    end
  end
  ...
  describe "slash mode — arg stage (/config kv slot)" do
    ...
    # The per-provider key list is browsable: tagged stage: :tool so the client
    # renders it as a selectable palette (masked secrets stay masked).
    it "tags the kv key slot stage: :tool (palette, not arg ghost)" do
      result = call(input: "/config google ", cursor: 15, authenticated: true)
      expect(result[:stage]).to eq(:tool)
    end
  ...
  describe "slash mode — arg stage (/config provider slot)" do
    it "suggests providers including sound after '/config ' (motion/fx removed — item 18)" do
      result = call(input: "/config ", cursor: 8, authenticated: true)
      child_labels = result[:menu_items].flat_map { |i| i[:children].map { |c| c[:label] } }
      expect(child_labels).to include("sound")
      expect(child_labels).not_to include("motion", "fx")
    end
  end

  # ── SLASH — unknown tool ─────────────────────────────────────────────────────

  describe "slash mode — unknown tool" do
  ...
  describe "free mode" do
    ...
    it "does not crash on any free-mode tool input" do
      %w[list ls show sync delete].each do |tool|
        expect { call(input: tool, cursor: tool.length, authenticated: true) }
          .not_to raise_error
      end
    end
  end

  # ── free mode — the TOOL stage (G75, 1.1.0) ─────────────────────────────────
  #
  # Before 1.1.0 the palette ignored the tool position entirely (arg-stage
  # only, G33): typing "l"/"lis"/"analy" popped nothing. Now the first word
  # prefix-filters the chat catalog, alias-aware (G75b): one row per tool,
  # labeled by the matched token (canonical preferred), inserting what the
  # user typed — "ls" stays "ls ", never rewritten to "list".

  describe "free mode — tool stage (G75)" do
    it "offers chat tools matching the typed prefix, stage :tool, alphabetical" do
    ...
    it "offers ONE row per tool even when several of its tokens match (G75b dedup)" do
    ...
    it "returns nothing for a prefix matching no tool" do
      expect(call(input: "zzz", cursor: 3, authenticated: true)[:menu_items]).to eq([])
    end

    it "returns nothing for anonymous visitors (every chat tool is auth-gated at dispatch)" do
  ...
  describe "free mode — slot progression (G32)" do
    ...
    it "drops an introducer whose pool is exhausted — app-wide gate, no per-tool code" do
      # analyze's `without` slot has exactly [numbers, breakdowns]; with both
      # committed, the gate has nothing behind it and must not be offered.
      expect(labels("analyze vid without numbers breakdowns ")).not_to include("without")
    end
  ...
  # upcoming/visibility filters) — they are not tools.yml slots, so after the
  ...
  describe "free-mode ghost" do
    ...
    # Unmatched tool — empty ghost
    it "returns empty ghost for unmatched tool 'frobnicate stuff'" do
  ...
  describe "hashtag mode for a live follow-up handle", :db do
    ...
    it "returns EMPTY_GHOST in the reply-tool position (no inline ghost)" do
      result = call(input: "#alpha-1266 ", cursor: 12, conversation:)
      expect(result[:ghost][:complete_current]).to eq("")
      expect(result[:ghost][:next_hint]).to eq("")
    end
  ...
  # ── Reply-tool PALETTE stage — the full allowed-tool set per reply_target ─────
  #
  # Regression guard: the engine must tag the reply-tool position stage: :tool and
  ...
  describe "hashtag follow-up: reply-tool palette stage", :db do
    let(:conversation) { Conversation.create! }
    ...
    it "tags the bare reply-tool position stage: :tool (palette, not ghost)" do
      stamp("vl-1", "video_list")
      result = call(input: "#vl-1 ", cursor: 6, conversation:)
      expect(result[:stage]).to eq(:tool)
    end

    it "tags a partially-typed reply tool stage: :tool" do
      stamp("vl-2", "video_list")
      result = call(input: "#vl-2 sh", cursor: 8, conversation:)
      expect(result[:stage]).to eq(:tool)
    end

    it "returns video_list's full tool set incl. schedule + with/without + shinies" do
    ...
    it "returns game_list's full tool set incl. with/without + shinies" do
    ...
    it "returns video_detail's tool set (rm/reindex/link/shinies)" do
    ...
    it "filters the palette by the typed tool prefix and keeps stage: :tool" do
    ...
    it "keeps stage: :arg once a NO-ARG tool is finalised (a second space)" do
    ...
    it "tags stage: :tool with argument items once an ARG tool is finalised (E13)" do
    ...
    it "each palette item inserts `<tool> ` (spliced over the partial tool token)" do
  ...
  #
  # After `#handle <tool> ` the palette suggests the tool's possible ARGUMENT
  # tokens for the source message's reply_target — config-driven from the tool's
  # declared reply branch (ref/args resolvers in config/pito/tools.yml).
  describe "hashtag follow-up: arg-stage argument suggestions", :db do
    ...
    context "columns — with/without on list targets" do
      ...
      it "tags a non-empty argument menu stage: :tool (browsable palette)" do
    ...
    context "metrics — with/without on analyze surfaces" do
      ...
      it "is STATE-AWARE: `with` offers only metrics the card does NOT display" do
        stamp("aa-3", "analyze_message",
              "analyze" => { "metric_keys" => %w[views subs], "with" => [], "without" => [] })
        expect(labels("#aa-3 with ", 11)).to eq([ "ctr", "watch time" ])
      end

      it "is STATE-AWARE: `without` offers only metrics the card DOES display" do
        stamp("aa-4", "analyze_message",
              "analyze" => { "metric_keys" => %w[views subs], "with" => [], "without" => [] })
        expect(labels("#aa-4 without ", 14)).to eq([ "subs", "views" ])
      end

      it "honors the stored selection — a metric already without'ed is addable again, not removable" do
        stamp("aa-5", "analyze_message",
              "analyze" => { "metric_keys" => %w[views subs], "with" => [], "without" => %w[subs] })
        expect(labels("#aa-5 with ", 11)).to include("subs")
        expect(labels("#aa-5 without ", 14)).to eq([ "views" ])
      end

      it "drops `with` from the TOOL palette when the card already shows every metric" do
        vocab = Pito::Grammar::Registry.vocabulary(:metrics)
        stamp("aa-6", "analyze_message",
              "analyze" => { "metric_keys" => vocab.canonical.map(&:to_s), "with" => [], "without" => [] })
        tool_labels = labels("#aa-6 ", 7)
        expect(tool_labels).not_to include("with")
        expect(tool_labels).to include("without")
      end
    end

    context "row ids — tools whose ref is a list row id" do
      before do
    ...
    context "no-arg / undeclared tools" do
      it "returns an empty :arg menu for a tool with no reply-branch config (share)" do
  ...
  # reply-tool palette.
  describe "hashtag follow-up: schedule tool for video_list", :db do
    let(:conversation) { Conversation.create! }
    let(:turn)         { conversation.turns.create!(input_kind: :slash, input_text: "/list videos", position: 1) }
    ...
    it "includes schedule in the reply-tool action palette" do
  ...
  # offered in the reply-tool palette for a video_detail reply.
  describe "hashtag follow-up: schedule offered for video_detail", :db do
    ...
    it "includes schedule (and the other video-card ops) in the reply-tool action palette" do
  ...
  # ── FREE MODE — chat tool slot palette suggestions (plan-0.9.5 E8/D5) ─────────
  #
  # Every chat tool's declared enum kwargs now autosuggest once the tool is
  ...
  describe "free mode — list/ls noun slot" do
    ...
    it "tags stage: :tool for the palette" do
      result = call(input: "list ", cursor: 5, authenticated: true)
      expect(result[:stage]).to eq(:tool)
    end
  ...
  describe "free mode — show full/with/only slots" do
    ...
    it "tags stage: :tool when suggestions are present" do
      result = call(input: "show game 5 ", cursor: 12, authenticated: true)
      expect(result[:stage]).to eq(:tool)
    end
  ...
  describe "free mode — tools with no suggestable slots yield empty menu_items" do
    ...
    it "returns empty for unknown tool 'frobnicate '" do
