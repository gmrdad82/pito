...
# Guard: follow-up reply availability is driven by tools.yml (the Matrix, via the
...
# on the detail cards: the list fell out of sync with tools.yml and rejected verbs
...
RSpec.describe "follow-up handlers: tools.yml drives availability (no hardcoded lists)" do
  handler_dir   = Rails.root.join("lib/pito/follow_up/handlers")
  ...
  handler_files.each do |file|
    ...
    it "#{name} gates availability via tools.yml, not an inline %w[...].include?(action) list" do
      ...
      expect(source).not_to match(FORBIDDEN_LIST_GATE),
        ...
        "so tools.yml stays the single source of truth for reply availability."
    end
  end
end
