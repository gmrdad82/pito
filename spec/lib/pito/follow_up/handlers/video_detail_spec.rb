...
RSpec.describe Pito::FollowUp::Handlers::VideoDetail, type: :service do
  ...
  describe "`@ai <text>` — anchored reply (owner-scoped roster)" do
    ...
    it "delegates to Chat::Handlers::Ai via ToolDelegator: a pending :ai event anchored on this card" do
      result = handler.call(event: source_event, rest: "@ai how is retention on this one", conversation:)
      ...
      expect(pending[:payload]["prompt"]).to eq("how is retention on this one")
  ...
  describe "#call — shinies" do
    ...
    it "appends the shinies message for the video" do
      payload = result.events.first[:payload]
  ...
  # tools.yml declares the `game` segment verb with a video_detail reply target, but
  ...
  # Every reply verb tools.yml declares for video_detail must reach the matrix-gated
  # ToolDelegator — a hardcoded allowlist here is what shadowed `game`. This locks
