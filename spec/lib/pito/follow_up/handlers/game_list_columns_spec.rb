...
RSpec.describe Pito::FollowUp::Handlers::GameList, "column mutations" do
  ...
  # A per-list-suppressed column (e.g. :channels on a single-channel result
  ...
  describe "#call on a channel-suppressed list" do
    ...
    it "rejects `with channel` — no error, same silent no-op as any unknown column" do
      ...
      expect(result).to be_a(Pito::FollowUp::Result::Mutation)
      expect(result.payload["list_columns"]).not_to include("channel", "channels")
    end
    ...
    it "`with platform` on the same suppressed list still works (other columns untouched)" do
      result = handler.call(event: suppressed_event, rest: "with platform", conversation:)
      expect(result.payload["list_columns"]).to include("genre", "platform")
      expect(result.payload["list_columns"]).not_to include("channel", "channels")
    end

    it "carries suppressed_columns forward on the rebuilt payload" do
      result = handler.call(event: suppressed_event, rest: "with platform", conversation:)
      expect(result.payload["suppressed_columns"]).to eq([ "channels" ])
    end
    ...
    context "when the list_cursor already carries suppressed_columns" do
      let(:cursor_event) do
        instance_double(Event, kind: "system", payload: suppressed_event_payload.merge(
          "list_cursor" => {
            "offset" => 1, "raw" => "list games", "channel" => nil,
            "sort_token" => nil, "sort_direction" => nil,
            "columns" => %w[genre], "suppressed_columns" => %w[channels]
          }
        ))
      end
      ...
      it "keeps suppressed_columns in the cursor after a with mutation" do
        result = handler.call(event: cursor_event, rest: "with platform", conversation:)
        expect(result.payload["list_cursor"]["suppressed_columns"]).to eq([ "channels" ])
      end
    end
  end
